...
# Sentinel: the previous-session lookup hit an expired JWT and already
# closed the websocket — the caller must stop the handshake.
PREV_LOOKUP_JWT_EXPIRED = object()


async def load_previous_session_data(*, project_id, user_id, raw, websocket):
    """Find the best previous chat session for a project (4-tier lookup).

    Extracted verbatim from websocket_agent (god-module split). Returns
    the _prev_session_data dict ({session_id, platform_repo_url,
    user_repo_url, user_repo_provider, generation_complete, messages}),
    None when no previous session exists, or PREV_LOOKUP_JWT_EXPIRED
    when the JWT expired mid-lookup (websocket already closed).
    """
    _prev_session_data: dict | None = None
    try:
