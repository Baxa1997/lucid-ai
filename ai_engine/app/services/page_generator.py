    ...
    async def _one(idx: int, section: dict[str, Any]) -> dict[str, Any] | None:
        async with sem:
            try:
                ...
                # Critical sections (hero / chrome / forms) get the
                # deterministic skeleton landing uses; anything else is
                # skipped and the page composes without it.
                logger.warning(
                ...
                try:
                    from app.services.landing_section_codegen import (
                        ...
                        _is_critical_section, _section_filename,
                    )
                    if _is_critical_section(section):
