...
async def gitlab_create_project(
    ...
    """Create a new GitLab project under a group/namespace.

    Returns the project dict with 'id', 'web_url', 'http_url_to_repo', etc.
    """
    import httpx
    ...
    payload = {
        ...
        "description": description or f"Generated by Lucid AI",
        "visibility": "private",
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        if resp.status_code in (200, 201):
            data = resp.json()
            logger.info("GitLab project created: %s (id=%s)", data.get("web_url"), data.get("id"))
            return data

        logger.error("GitLab create project failed (%d): %s", resp.status_code, resp.text[:300])
        raise RuntimeError(f"GitLab create project failed ({resp.status_code}): {resp.text[:200]}")


async def gitlab_add_member(
    gitlab_url: str,
    project_id: int,
    token: str,
    user_id: int = 0,
    username: str = "",
    access_level: int = 50,  # 50 = Owner
) -> bool:
    """Add a user as a member to a GitLab project.

    Access levels: 10=Guest, 20=Reporter, 30=Developer, 40=Maintainer, 50=Owner
    Either user_id or username must be provided. If username is given, we look up the ID first.
    """
    import httpx
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        ...
        if not user_id and username:
            search_url = f"{gitlab_url}/api/v4/users?username={username}"
            resp = await client.get(search_url, headers=headers)
            if resp.status_code == 200:
                users = resp.json()
                if users:
                    user_id = users[0]["id"]
                    logger.info("Resolved GitLab username '%s' → user_id %d", username, user_id)
                else:
                    logger.error("GitLab user '%s' not found", username)
                    return False
            else:
                logger.error("GitLab user search failed (%d): %s", resp.status_code, resp.text[:200])
                return False

        if not user_id:
            logger.error("No user_id or username provided for GitLab member invite")
            return False
        ...
        if resp.status_code in (200, 201):
            logger.info("GitLab member added: user_id=%d to project %d (access=%d)", user_id, project_id, access_level)
            return True

        if resp.status_code == 409:
            # Already a member — try to update access level
            update_url = f"{url}/{user_id}"
            resp2 = await client.put(update_url, headers=headers, json={"access_level": access_level})
            if resp2.status_code in (200, 201):
                logger.info("GitLab member updated: user_id=%d access=%d", user_id, access_level)
                return True
            logger.warning("GitLab member update failed: %s", resp2.text[:200])
            return False

        logger.warning("GitLab add member failed (%d): %s", resp.status_code, resp.text[:200])
        return False
...
async def setup_project_cicd(
    ...
    if gitlab_token and gitlab_project_id:
        ...
        # ── Step F2: Add CI/CD user as project Owner ───────────
        # Required for deployment pipeline to have access
        try:
            cicd_username = os.environ.get("GITLAB_CICD_USERNAME", "udevs")
            if cicd_username:
                await _ws_progress(f"👤 Adding '{cicd_username}' as project Owner...")
                member_ok = await gitlab_add_member(
                    gitlab_url=gitlab_url,
                    project_id=int(gitlab_project_id),
                    token=gitlab_token,
                    username=cicd_username,
                    access_level=50,  # Owner
                )
                if member_ok:
                    await _ws_progress(f"✅ '{cicd_username}' added as Owner")
                else:
                    await _ws_progress(f"⚠️ Could not add '{cicd_username}' — add manually in GitLab")
        except Exception as e:
            logger.error("Step F2 (add member) failed: %s", e)
            await _ws_progress(f"⚠️ Member invite failed: {e}")
