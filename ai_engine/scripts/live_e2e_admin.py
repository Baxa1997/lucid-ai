"""Admin/CRM e2e validation — react-admin template, multi-entity dashboard.

Mirrors live_e2e_landing.py but exercises the admin generation path:
  • react-admin template (Vite + React, sidebar + multi-page CRUD)
  • Phase 2 entity batching (parallel via _run_phase2_parallel_batches)
  • Phase 2.5 entity coverage audit
  • Authoritative-paths filter applied to admin batches

Run from ai_engine/:
    venv/bin/python scripts/live_e2e_admin.py 2>&1 | tee /tmp/lucid_admin.log
"""
...
async def main():
    ...
    tmp = tempfile.mkdtemp(prefix="lucid_admin_")
    print(f"\nWorkspace: {tmp}")

    print(f"Cloning admin template…")
    ...
    description = (
        "An admin dashboard for LeafMail, a SaaS email marketing platform. "
        "Manage entities: Campaigns (name, subject, status, sent_count, open_rate), "
        "Subscribers (email, name, list, status, joined_at), Lists (name, "
        "subscribers_count, created_at), and Templates (name, category, last_used). "
        "Include a dashboard with KPI cards (total subscribers, campaigns sent, "
        "open rate, bounce rate), recent activity table, and revenue/engagement charts."
    )
    ...
    validated = {
        ...
        "project_stack": "react-admin",
        "skeleton_stack": "react-admin",
        "skeleton_name": "react-admin",
        "is_admin": True,
        "scratch_mode": False,
    ...
    print("=" * 70)
    print("LAUNCHING — generate_new_project (admin/CRM, real Anthropic + Gemini)")
    print("=" * 70)
    ...
    for d in expected_dirs:
        ...
        contents = os.listdir(full) if ok else []
        print(f"  {'✓' if ok else '✗'} {d}/ ({len(contents)} entries)")

    # Look for entity-specific files
    print("\n📋 Entity coverage:")
    entities = ["campaigns", "subscribers", "lists", "templates"]
    ...
    if os.path.isdir(src):
        ...
        for ent in entities:
            matches = [f for f in all_files if ent.lower() in os.path.basename(f).lower()]
            print(f"  {'✓' if matches else '✗'} {ent}: {len(matches)} file(s)")
