...
import {
  ...
  Clock, Hash, Github, Trash2, X, AlertTriangle, Check,
  GitBranch,
} from 'lucide-react';
import { useState, useEffect, useCallback } from 'react';
import { cn } from '@/lib/utils';
import { useRouter } from 'next/navigation';
import { listConversations, deleteConversation, deleteConversations } from '@/lib/conversations';
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-[#151b23] rounded-2xl border border-slate-200 dark:border-slate-700/50 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-end gap-3 px-6 pb-6">
          <button onClick={onCancel} disabled={loading} className="px-4 py-2.5 text-sm font-semibold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-white/[0.06] hover:bg-slate-200 dark:hover:bg-white/[0.1] rounded-xl border border-slate-200 dark:border-slate-700/50 transition-all">
            Cancel
          </button>
...
/* ═══════════════════════════════════════════════════
   CONVERSATION ROW
   ═══════════════════════════════════════════════════ */
function ConversationRow({ conversation, selected, onSelect, onClick, onDelete }) {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        selected && "bg-blue-50/50 dark:bg-blue-500/5 hover:bg-blue-50 dark:hover:bg-blue-500/5"
      )}
    >
      {/* Checkbox */}
      <div
      ...
      <div className="shrink-0 bg-[#f5f7fa] dark:bg-[#0d1117] px-6 lg:px-8 pt-6 pb-4">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">Conversations</h1>
              <p className="text-[12px] text-slate-400 dark:text-slate-500 font-medium mt-0.5">
                {conversations.length} workspace{conversations.length !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
          <button
            ...
            {filtered.length > 0 && (
              <div className="flex items-center gap-4 px-4 py-2.5 border-b border-slate-100 dark:border-[#21262d] bg-slate-50 dark:bg-[#0f1118]">
                ...
                >
                  ...
                  {someSelected && !allSelected && <div className="w-2 h-0.5 bg-blue-600 dark:bg-blue-400 rounded-full" />}
                </div>
                <span className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider flex-1">
                  {selected.size > 0 ? `${selected.size} selected` : `${filtered.length} conversation${filtered.length !== 1 ? 's' : ''}`}
                </span>
                <span className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider shrink-0">Updated</span>
            ...
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center px-6">
                ...
                <h3 className="text-[15px] font-bold text-slate-900 dark:text-slate-100 mb-1">
                  {conversations.length === 0 ? 'No conversations yet' : 'No matching results'}
                </h3>
