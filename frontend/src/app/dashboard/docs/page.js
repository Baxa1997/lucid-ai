  ...
  return (
    <div className="h-screen flex flex-col bg-white dark:bg-[#0d1117] overflow-hidden font-sans text-slate-900 dark:text-slate-100 transition-colors duration-200">
      ...
      <header className="h-16 border-b border-slate-100 dark:border-slate-800 flex items-center px-6 justify-between bg-white dark:bg-[#0d1117] shrink-0 z-30 relative transition-colors duration-200">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h1 className="font-bold text-sm tracking-tight text-slate-900 dark:text-slate-100">Lucid AI Documentation</h1>
        ...
        <div className="flex items-center gap-4">
           ...
           <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center border border-slate-200 dark:border-slate-700 cursor-pointer">
             <div className="w-full h-full rounded-full overflow-hidden bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-600 dark:to-slate-700"></div>
           </div>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <div className="w-[280px] flex flex-col border-r border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/50 shrink-0 transition-colors duration-200">
          ...
          <div className="flex-1 overflow-y-auto p-4">
             <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3 pl-2">Table of Contents</div>
             ...
             <ul className="space-y-0.5">
               ...
               <div className="pt-2 pb-1">
                 <div className="flex items-center justify-between px-2 py-1 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded cursor-pointer group">
                    <span className="text-xs font-semibold flex items-center gap-1">
                      <ChevronDown className="w-3 h-3" /> Core Concepts
                    </span>
                    <Plus className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" />
                 </div>
                 <div className="pl-4 border-l border-slate-200 dark:border-slate-700 ml-3 mt-1 space-y-0.5">
          ...
          <div className="p-4 border-t border-slate-100 dark:border-slate-800">
             <div className="text-xs text-slate-400 dark:text-slate-500 flex items-center gap-2 px-2">
                <LayoutTemplate className="w-3.5 h-3.5" />
                <span>Theme: </span>
                <span className="font-medium text-slate-600 dark:text-slate-300">GitBook Light</span>
             </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-white dark:bg-[#0d1117] relative transition-colors duration-200">
          ...
          <div className="max-w-4xl mx-auto px-12 py-12">
            ...
            <div className="prose prose-slate dark:prose-invert prose-lg max-w-none">
              ...
              <div className="bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 mb-8 font-mono text-xs overflow-x-auto relative group/code">
                <div className="absolute right-4 top-4 opacity-0 group-hover/code:opacity-100 transition-opacity">
                  <button className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"><Share2 className="w-4 h-4" /></button>
                </div>
                <div className="flex gap-1.5 mb-3">
                   <div className="w-2.5 h-2.5 rounded-full bg-slate-300 dark:bg-slate-600"></div>
                   <div className="w-2.5 h-2.5 rounded-full bg-slate-300 dark:bg-slate-600"></div>
                </div>
                <span className="text-purple-600 dark:text-purple-400">const</span> <span className="text-blue-600 dark:text-blue-400">authenticate</span> = <span className="text-purple-600 dark:text-purple-400">async</span> (apiKey) ={'>'} {'{'}
                <br/>
                &nbsp;&nbsp;<span className="text-purple-600 dark:text-purple-400">const</span> response = <span className="text-purple-600 dark:text-purple-400">await</span> fetch(<span className="text-green-600 dark:text-green-400">'https://api.lucid.ai/v1/auth'</span>, {'{'}
                <br/>
                &nbsp;&nbsp;&nbsp;&nbsp;method: <span className="text-green-600 dark:text-green-400">'POST'</span>,
                <br/>
                &nbsp;&nbsp;&nbsp;&nbsp;headers: {'{'} <span className="text-green-600 dark:text-green-400">'Authorization'</span>: <span className="text-green-600 dark:text-green-400">{`\`Bearer \$\{apiKey\}\``}</span> {'}'}
                <br/>
                &nbsp;&nbsp;{'}'});
                <br/>
                &nbsp;&nbsp;<span className="text-purple-600 dark:text-purple-400">return</span> response.json();
                <br/>
                {'}'};
              </div>

              <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mt-10 mb-4">Handling Errors</h2>
              <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
                If the token is invalid or expired, the API will return a <code className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-sm text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">401 Unauthorized</code> response. 
        ...
        <div 
          className={cn(
            "absolute top-0 right-0 h-full w-[320px] bg-white dark:bg-slate-900 border-l border-slate-100 dark:border-slate-800 shadow-2xl transform transition-transform duration-300 ease-in-out z-40",
            showPublishPanel ? "translate-x-0" : "translate-x-full"
          )}
        >
          <div className="p-6 h-full flex flex-col">
             <div className="flex items-center justify-between mb-8">
                <h3 className="font-bold text-slate-900 dark:text-slate-100 text-lg">Publish Settings</h3>
                <button onClick={() => setShowPublishPanel(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                   <X className="w-5 h-5" />
                </button>
             </div>
             ...
             <div className="space-y-8 flex-1">
                ...
                <div className="space-y-4">
                   <div className="flex items-start justify-between">
                      <div className="flex gap-3">
                         ...
                         <div>
                            <div className="font-semibold text-sm text-slate-900 dark:text-slate-100">Custom Domain</div>
                            <div className="text-[11px] text-slate-500 dark:text-slate-400 leading-tight mt-1 max-w-[150px]">Host docs on your own domain (e.g. docs.yours.com)</div>
                         </div>
                      </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-sm text-slate-900 dark:text-slate-100">Export as PDF</div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 leading-tight mt-1">Allow users to download docs</div>
                      </div>
                  </div>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex gap-3">
                      <div className="w-8 h-8 rounded-lg bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-600 dark:text-amber-400 mt-0.5">
                        <Code className="w-4 h-4" />
                      </div>
                      <div>
                        <div className="font-semibold text-sm text-slate-900 dark:text-slate-100">Generate SDKs</div>
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 leading-tight mt-1">Auto-build client libraries</div>
                      </div>
                  </div>
                  <ToggleSwitch checked={true} />
                </div>
             ...
             <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800">
                <button className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold py-3 rounded-xl hover:bg-slate-800 dark:hover:bg-slate-100 transition-all shadow-lg flex items-center justify-center gap-2 text-sm">
                   <Zap className="w-4 h-4 text-yellow-400 dark:text-yellow-500 fill-current" />
                   Publish Changes now
                </button>
             </div>
          </div>
